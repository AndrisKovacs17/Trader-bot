...
class ExecutionOrderEventHandler(IEventHandlerPort):
    ...
    async def handle(self, event: Event) -> None:
        if isinstance(event, OrderFilledEvent):
            applied = await self._apply_fill_side_effects(event)
            if not applied:
                return
    ...
    async def _apply_fill_side_effects(self, event: OrderFilledEvent) -> bool:
        ...
        try:
            ...
            logger.warning("Ignoring invalid fill event for %s: %s", symbol, error)
            return False
