...
\begin{description}
  \item[\textbf{Fő folyamat} (\texttt{main.py})]
  ...
  \item[\textbf{aiohttp webes szerver} (\texttt{adapters/web/dashboard.py})]
  ...
  \item[\textbf{Binance WebSocket / REST} (külső)]
...
A Kalman-diagnosztikai nézet (\texttt{kcmamba\_kalman.html}) a modell belső
...
\begin{figure}[H]
  ...
  \fbox{\begin{minipage}[c][6cm][c]{0.92\textwidth}
    \centering
    \textit{[Képernyőkép: Kalman-szűrő diagnosztika, \texttt{kcmamba\_kalman.html}]}\\[4pt]
...
Az alkalmazás a \texttt{main.py} belépési ponton keresztül kezelhető
(ld.~\ref{sec:running}. alfejezet). Az irányítópult a \texttt{http://localhost:8080}
címen érhető el; a visszatesztelési eredmények CSV-formátumban exportálhatók,
a naplók a \texttt{run\_log.txt} fájlban találhatók.

\subsection{Hibaelhárítás}

\begin{description}
  \item[\texttt{ModuleNotFoundError: mamba\_ssm}] Telepítsd a
       \texttt{requirements-ml.txt}-ben szereplő CUDA-kompatibilis csomagokat,
       vagy futtasd CPU-n: \texttt{python main.py --device cpu}.
  \item[Binance API 403/401] Ellenőrizd a \texttt{.env} fájl kulcsait és az
       IP-whitelist beállításait a Binance kereskedési felületén.
  \item[WebSocket kapcsolódási hiba] A tűzfal blokkolhatja a Binance \texttt{wss://}
...
\paragraph{Kockázati szabályok} A \texttt{test\_risk\_policy.py} ellenőrzi,
hogy az összes tiltó szabály (MaxQtyRule, MaxNotionalRule, ConfidenceRule,
SlippageGuardRule) helyesen blokkol érvénytelen megbízásokat, és hogy a
\texttt{RiskPolicy} együttes alkalmazásakor a legszigorúbb szabály érvényesül.
A tesztek véletlenszerű bemenetek mellett (~100 kombináció) is megbízhatóan
lefedik a határeseteket.

\paragraph{Stratégia jelzéslogika} A \texttt{test\_strategy.py} verifikálja,
hogy a \texttt{ThresholdStrategy} pontosan akkor ad \textsc{Buy}/\textsc{Sell}
jelzést, ha az előrejelzett valószínűség meghaladja a beállított küszöböt
(\texttt{threshold}), és hogy a \texttt{min\_edge}, \texttt{min\_confidence}
és \texttt{max\_sigma} paraméterek megfelelően szűrik a gyenge jelzéseket.

\paragraph{Tőkeszámítás és kitöltési logika} A \texttt{test\_equity.py} és
\texttt{test\_fill\_fix.py} összes tesztje zöld, ami megerősíti, hogy a
PnL-számítás és a részleges kitöltések kezelése helyesen működik multi-szimbólumos
portfóliókban.

\paragraph{Regressziós tesztek} A \texttt{test\_fee\_fix.py} garantálja, hogy
egy korábbi bugfix (jutalék duplán számolása tételes nézeten) nem kerül vissza
a kódbázisba.
