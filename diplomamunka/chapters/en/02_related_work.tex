...
$y = \bar{C} * u$ can be computed as a single element-wise product after a
Fast Fourier Transform (FFT), reducing sequence-level complexity to $O(T \log T)$.
...
S4 efficient, because there is no longer a single kernel to transform via FFT. Gu and Dao
...
is $O(L \log L)$ via FFT and provides an inductive bias well suited to periodic
data.
