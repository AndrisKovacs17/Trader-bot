...
The key idea is that rather than using a fixed forgetting factor $A$, the model
...
\begin{figure}[H]
  ...
  \caption{Full data-flow diagram of the KCMamba block. Yellow blocks
           ...
           in $O(\log T)$ synchronisation steps; green blocks denote
...
\begin{figure}[H]
  ...
  \caption{Blelloch parallel prefix-scan on a length-8 sequence. Each pass
           combines two elements separated by the current step size; after
...
\begin{figure}[H]
  ...
  \caption{Synthetic signal filtering: \textbf{vanilla SSM} (standard Mamba,
           ...
           \textit{Top rows:} predicted vs.\ true signal; KCMamba tracks
           more tightly even under noise.
           \textit{Bottom rows:} learned $K$ values over time.
           Vanilla SSM has a fixed $A$; KCMamba's $A$ adapts with noise level.}
...
\begin{figure}[H]
  ...
  \caption{\textbf{Left:} Impulse-response function: exponential state decay
           ...
           horizons; KCMamba automatically extends memory under noise.}
...
\begin{table}[H]
  ...
  \caption{Module and class structure of the system. Each class depends only
           on the data types and interfaces of its own package; cross-package
