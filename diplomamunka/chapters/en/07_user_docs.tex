...
\begin{enumerate}
  \item Create a virtual environment in the project folder:
\begin{verbatim}
cd diplomamunkakod
python -m venv .venv
# Windows (PowerShell):
...
\end{verbatim}
  ML packages (optional, for GPU benchmarks):
\begin{verbatim}
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{4.4cm}|p{2.8cm}|p{6.2cm}|}
    ...
    \hline
    \texttt{ModuleNotFoundError: pandas} & Error & The venv cannot see system packages; run: \texttt{pip install --ignore-installed -r requirements.txt} \\
    \hline
