...
\subsubsection*{Why does KCMamba generalise better with scarce data?}
...
\begin{itemize}
    \item \textbf{KCMamba}: at $\sigma{=}3$ the MSE drops from $0.673$ to $0.466$
...
\begin{itemize}
    \item \textbf{$s{=}1$, $\sigma{=}3$}: KCMamba wins consistently from $T{=}96$
    ...
    \item \textbf{$s{=}16$, $\sigma{\geq}3$}: Fair Mamba completely collapses
          ...
          KCMamba is the only architecture that does not catastrophically collapse.
\end{itemize}
...
\begin{figure}[H]
  ...
  \caption{MSE vs.\ sequence length for dense ($s{=}1$, top row) and sparse
           ...
           $8.1$--$8.5$).
           KCMamba is stable in every configuration; Fair Mamba catastrophically
